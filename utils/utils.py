...
class BBoxUtility(object):
    ...
    def detection_out(self, predictions, mbox_priorbox, num_classes,
            ...
            for c in range(num_classes):
                ...
                if len(c_confs[c_confs_m]) > 0:
                    ...
                    idx = tf.image.non_max_suppression(boxes_to_process, confs_to_process, 
                                                        self._top_k, iou_threshold=self._nms_thresh).numpy()
                    # 取出在非极大抑制中效果较好的内容
    ...
    def nms_for_out(self,all_labels,all_confs,all_bboxes,num_classes,nms):
        ...
        for c in range(num_classes):
            ...
            if len(all_confs[mask]) > 0:
                ...
                idx = tf.image.non_max_suppression(boxes_to_process, confs_to_process, self._top_k, iou_threshold=nms).numpy()
                # 取出在非极大抑制中效果较好的内容
                good_boxes = boxes_to_process[idx]
                confs = confs_to_process[idx][:, None]
                # 将label、置信度、框的位置进行堆叠。
